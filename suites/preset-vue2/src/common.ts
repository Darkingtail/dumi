...
export default (api: IApi) => {
  ...
  api.modifyConfig((memo) => {
    ...
    const vueConfig = userConfig?.vue; // Read from 'vue' config key
    ...
    if (!options.entryFile || !options.resolveDir) {
      return memo;
    }

    console.log('[preset-vue2] Registering atomParser in modifyConfig...');
